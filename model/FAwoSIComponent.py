...
class FAwoSIBlock(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        super(FAwoSIBlock,self).__init__()
        
        self.n_feats = n_feats
        self.norm1 = LayerNorm(n_feats)
        self.mixer = FAwoSImixer(n_feats,window_size=window_size,ratio=ratio)
...
class FAwoSIGroup(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        super(FAwoSIGroup, self).__init__()
        ...
        self.body = nn.ModuleList([FAwoSIBlock(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
