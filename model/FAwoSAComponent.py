...
class FABlock_woSA(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        super(FABlock_woSA,self).__init__()
        
        self.n_feats = n_feats
        self.norm1 = LayerNorm(n_feats)
        self.mixer = FAmixer_woSA(n_feats,window_size=window_size,ratio=ratio)
...
class FAGroup_woSA(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        super(FAGroup_woSA, self).__init__()
        ...
        self.body = nn.ModuleList([FABlock_woSA(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
