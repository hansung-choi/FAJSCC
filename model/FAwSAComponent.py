...
class FAMixer_wSA(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5):
        ...
        k = 3
        d = 2
        self.snr_feature_refine = nn.Conv2d(8, dim, 1, 1, 0, bias = bias)
    ...
    def forward(self,x,condition_global=None,snr_feature=None, mask=None, train_mode=False):
        N,C,H,W = x.shape
        
        sf = self.snr_feature_refine(snr_feature)
        x = x + sf # combine x with snr feature information
        v = self.project_v(x)
...
class FABlock_wSA(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        super(FABlock_wSA,self).__init__()
        
        self.n_feats = n_feats
        self.norm1 = LayerNorm(n_feats)
        self.mixer = FAMixer_wSA(n_feats,window_size=window_size,ratio=ratio)
        self.norm2 = LayerNorm(n_feats)
        self.ffn = GatedFeedForward(n_feats)
        
    def forward(self,x,condition_global=None,snr_feature=None):
        if self.training:
            res, decision = self.mixer(x,condition_global,snr_feature) #decision.size() = B X 1, [ratio of predicted 1 of first image,...,...of B'th image]
            x = self.norm1(x+res)
            res = self.ffn(x)
            x = self.norm2(x+res)
            return x, decision
        else:
            res = self.mixer(x,condition_global,snr_feature)
            x = self.norm1(x+res)
            res = self.ffn(x)
            x = self.norm2(x+res)
            return x 
...
class FAGroup_wSA(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        super(FAGroup_wSA, self).__init__()
        ...
        self.snr_embedding = nn.Sequential(nn.Conv2d(1, 8, 1, 1, 0, bias=True),
                                        nn.LeakyReLU(negative_slope=0.1, inplace=True),
                                        nn.Conv2d(8, 8, 1, 1, 0, bias=True),
                                        nn.LeakyReLU(negative_slope=0.1, inplace=True))


        self.body = nn.ModuleList([FABlock_wSA(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
        self.body_tail = nn.Conv2d(n_feats, n_feats, 1, 1, 0)
        
    def forward(self,x,snr): #snr: single integer or float
        decision = []
        condition_global = self.global_predictor(x)
        N,C,H,W = x.shape
        snr_tensor = torch.ones(N,1,1,1) * snr
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        snr_tensor = snr_tensor.to(device).float()
        snr_feature = self.snr_embedding(snr_tensor)
        shortcut = x.clone()
        if self.training:
            for _, blk in enumerate(self.body):
                x, mask = blk(x,condition_global,snr_feature) #mask.size() = B X 1, [ratio of predicted 1 of first image,...,...of B'th image]
                decision.append(mask)
            x = self.body_tail(x) + shortcut
            return x, decision #decision = [mask at first block, ..., mask at n_block'th block]
        else:
            for _, blk in enumerate(self.body):
                x = blk(x,condition_global,snr_feature)
            x = self.body_tail(x) + shortcut
            return x 
